...
// import './style.less';
// import brush from '@ipaint/brush';
import {
...
export class Brush {
  ...
  public render() {
    ...
    this._component.innerHTML = `
      <div class="ipaint-board-brush-container">
        <div class="ipaint-board-brush-content">
          ${brushNameList.map((name: BrushNameType) => {
            return `
            <div class="ipaint-board-brush-item" data-brush-name="${name}"></div>
            `;
          }).join('\r\n')}
        </div>
      </div>
    ...
    brushNameList.forEach((name) => {
      ...
      if (elem) {
        brushMap[name].classList.add('ipaint-board-brush-img')
